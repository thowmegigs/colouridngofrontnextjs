...
export default function ProductCarouselCollection({section }: ProductCarouselProps) {
  const scrollRef = useRef<HTMLDivElement>(null)
 
  const [visibleItems, setVisibleItems] = useState(4)
  const [autoScroll, setAutoScroll] = useState(true)
  const [currentIndex, setCurrentIndex] = useState(0)
const products=section.collection_products_when_single_collection_set;
...
const sub_title=section.section_subtitle;
  ...
  return (
    <div className="relative">
        ...
        <div
          ref={scrollRef}
          className={`relative overflow-hidden ${isHorizontal ? "flex gap-1 pb-4 overflow-x-auto scrollbar-hide" : "grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-1"}`}
