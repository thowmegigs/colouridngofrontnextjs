...
export default function ProductDetail({ slug }: ProductDetailProps) {
  ...
  useEffect(() => {
    if (product)
      setAvailableCoupons(product.related_coupons)
  }, [product])
                  ...
                  {availableColorsForSize.map((color) => (
                    <div key={color.id} className="relative">
                      <Label
                        ...
                        className={`relative w-10 h-10 rounded-full cursor-pointer flex items-center justify-center border-2 ${!color.available
                            ? "opacity-50"
                  ...
                  {availableSizesForColor.map((size) => (
                    <div key={size.id} className="relative">
                      <Label
                        ...
                        className={`border rounded-md px-4 py-2 cursor-pointer text-sm ${!size.available
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-border">
                <tbody className="divide-y divide-border">
                  ...
                  {product.sleeve_type && (
                    <tr>
                      <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground w-1/3">Sleeve Type</th>
                  ...
                  {product.waist_rise && (
                    <tr>
                      <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground w-1/3">Waist Rise</th>
                  ...
                  {product.surface_style && (
                    <tr>
                      <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground w-1/3">Surface Style</th>
                  ...
                  {product.transparency && (
                    <tr>
                      <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground w-1/3">Transparency</th>
          ...
          <TabsContent value="reviews" className="space-y-6 mt-6">
            {product.reviews.length > 0 ? (
              <div className="flex flex-col md:flex-row gap-8">
                <div className="md:w-1/3">
                  <div className="border rounded-lg p-6 space-y-4">
                    <div className="text-center">
                      ...
                      <div className="flex justify-center my-2">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            key={i}
                            className={`h-5 w-5 ${i < Math.floor(product.rating) ? "text-amber-500 fill-current" : "text-muted-foreground"
                              }`}
                          />
                        ))}
                      </div>
                    ...
                    <Button className="bg-primary text-white w-full" onClick={() => setShowReviewForm(true)}>
                      Write a Review
                    </Button>
                  </div>
                </div>
                  ...
                  {product.reviews.map((review:any) => (
                    <div key={review.id} className="border rounded-lg p-6">
                      <div className="flex justify-between mb-2">
                        <div className="font-medium">{review.user_name??'Genuine User'}</div>
                        <div className="flex">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
              ...
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <Button className="bg-primary text-white" onClick={() => setShowReviewForm(true)}>
                  Write a Review
                </Button>
              </div>
            )}
