...
export default function CategoryPage({ slug }: CategoryPageProps) {
  ...
  useEffect(() => {
    if (facet_options) {
      ...
      const facetFilterSections = Object.keys(facet_options).map(key => ({
        key,
        title: key
          .split('_')
          .map(word => word.charAt(0).toUpperCase() + word.slice(1))
          .join(' '),
        options: facet_options[key]
      }))

      setFacetOptions(facetFilterSections)
    }
  }, [facet_options])
  ...
  const FilterContent = useCallback(() => {
    ...
    let filterSections = [
      ...
      { key: "colors", title: "Colors", options: filterOptions.colors },
      { key: "age_groups", title: "Age Groups", options: filterOptions.age_groups },
    ]
    filterSections = [...filterSections, ...facetOptions];
